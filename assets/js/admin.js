...
var handleSubmit = function(e) {
    ...
    query.done(function (data) {
        ...
        hideDatabaseWarning();
        React.render(
            React.createElement(Results, {series: series}),
...
var Results = React.createClass({
  render: function() {
    var results = this.props.series.map(function(series) {
		var r = [];
		if ($('#enable-graph-checkbox')[0].checked) {
			r.push(React.createElement(Graph, {data: series}));
		}
		if ($('#enable-table-checkbox')[0].checked) {
            r.push(React.createElement("table", {className: "table"},
...
var Graph = React.createClass({
	getInitialState: function() {
		return {separateByCol: null};
	},

    render: function() {
    	if (this.props.data.values.length == 0) {
    		return React.createElement("p", {className: "error"}, "Cannot plot graph: no data for this series");
    	}
        
        return React.createElement("div", {className: "graph-main", ref: "graphMainDiv"}, 
        	React.createElement("div", {className: "graph", ref: "graphDiv"}),
        	React.createElement("div", {className: "legend", ref: "legendDiv"}),
        	React.createElement(TagSelect, {data: this.props.data, onChange: this.updateTagSelect})
        );
    },
    ...
    draw: function() {
    	var iTime = this.props.data.columns.indexOf('time');
    	if (iTime == -1) {
    		$(this.refs.graphMainDiv.getDOMNode()).html('<p class="error">Cannot plot graph: no timestamps in data</p>');
    		return;
    	}

		// Extract a set of lines to plot, indexed by the series name for the legend
		var dataBySeries = {};
		var columns = this.props.data.columns;
		
		var separateByCol = this.state.separateByCol;
		$.each(this.props.data.values, function(iRow, row) {
			var t = new Date(row[iTime]).getTime();
			$.each(columns, function(iCol, colName) {
				if (iCol == iTime) {
					return;
				}
				if (typeof(row[iCol]) != 'number') {
					return;
				}
				if (separateByCol === null) {
					seriesName = colName;
				} else {
					seriesName = colName + '.' + row[separateByCol];
				}
				if (dataBySeries[seriesName] === undefined) {
					dataBySeries[seriesName] = [];
				}
				dataBySeries[seriesName].push([t, row[iCol]]);
			});
        });

		// Reformat into something we can pass to Flot
    	var graphData = [];
		$.each(dataBySeries, function(label, data) {
			graphData.push({
				data: data,
				label: label
			});
    	...
    	var graphOptions = {
			...
			legend: {
				show: true,
				container: $(this.refs.legendDiv.getDOMNode()),
				noColumns: 1,
			...
			tooltip: {
				...
				content: function(label, xval, yval, flotItem) {
					return label + " at " + new Date(xval).toISOString() + " = " + (yval).toString(); 
				}
			},
			xaxis: {
				mode: "time"
    	...
    	var flotObject = $.plot($(this.refs.graphDiv.getDOMNode()), graphData, graphOptions);
    	
		// If someone selects a portion, zoom in
		$(this.refs.graphDiv.getDOMNode()).bind("plotselected", function (event, ranges) {
			...
			flotObject.getOptions().yaxes[0].min = ranges.yaxis.from;
			flotObject.getOptions().yaxes[0].max = ranges.yaxis.to;
			flotObject.setupGrid();
			flotObject.draw();
		});

		// If someone double clicks the plot, reset to default view
		$(this.refs.graphDiv.getDOMNode()).dblclick(function reset() {
			flotObject.getOptions().xaxes[0].min = null;
			flotObject.getOptions().xaxes[0].max = null;
			flotObject.getOptions().yaxes[0].min = null;
			flotObject.getOptions().yaxes[0].max = null;
			flotObject.setupGrid();
			flotObject.draw();
		});    	
	},
	
	updateTagSelect: function(event) {
		var value = event.target.value;
		if (value == "") {
			value = null;
		}
		this.setState({separateByCol: value}, this.draw);
	}
});

var TagSelect = React.createClass({
	// A dropdown list of tag-like columns.  We actually allow any string or
	// boolean columns, so some of them may actually be fields, but hopefully
	// the cardinality will usually be low enough not to explode things.
	render: function() {
        var tagCols = [];
	    var values = this.props.data.values;
	    $.each(this.props.data.columns, function(iCol, colName) {
			if (colName == 'time') {
				return;
			}
			for (var row=0; row<values.length; ++row) {
				// Scan down the column looking for the first non-null value,
				// to find the datatype of the column.
				if (values[row][iCol] !== null) {
					if (typeof(values[row][iCol]) == 'string' || typeof(values[row][iCol]) == 'boolean') {
						tagCols.push({index: iCol, name: colName});
					}
					return;
				}
			}
        ...
        if (tagCols.length == 0) {
        	return null;  // no tags to select, no point displaying anything
        }

        return React.createElement("div", {className: "tag-select form-inline"},
			React.createElement("div", {className: "form-group"},
				React.createElement("label", null, "Separate plots by: "),
				React.createElement("select", {className:"form-control", onChange: this.props.onChange},
					React.createElement("option", {value: ""}, ""),
					tagCols.map(function(col) {
						return React.createElement("option", {value: col.index}, col.name);
					})
				)
			)
        );
...
var getClientVersion = function () {
	$('.influxdb-client-version').html("v1.0");
}
...
var getDatabases = function () {
    ...
    query.done(function (data, status, xhr) {
        ...
        if (version.indexOf("unknown") == -1) {
            version = "v" + version;
            $('#influxdb-doc-link').attr('href', 'https://docs.influxdata.com/influxdb/v'+truncateVersion(version)+'/introduction/getting_started/');
        }
